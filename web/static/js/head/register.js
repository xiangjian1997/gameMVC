...
function checkLogin() {
    ...
    if (checkN && checkP) {
        flag = Sigin($("#input_username").val(), $("#ali213SSO_input_pwd").val());
    }
    return flag;
}
...
function checkUserName() {//检查用户名
    ...
    if (userName != "" && userName != "用户名/手机/邮箱") {
        if (checkPhone.test(userName) || checkEmail.test(userName) || checkUser.test(userName)) {
            $("#input_username").next().html("");
            return true;
        } else {
            $("#input_username").next().html("用户名格式不正确");
            return false;
        }
    } else {
        $("#input_username").next().html("用户名不能为空");
        return false;
    }
}
...
function checkUserPassword() {//检查用户名
    ...
    if (userPassword != "" && userPassword != "密码") {
        if (checkPassw.test(userPassword)) {
            $("#ali213SSO_input_pwd").next().html("");
            return true;
        } else {
            $("#ali213SSO_input_pwd").next().html("密码格式不正确");
            return false;
        }
    } else {
        $("#ali213SSO_input_pwd").next().html("密码不能为空");
        return false;
    }
}
